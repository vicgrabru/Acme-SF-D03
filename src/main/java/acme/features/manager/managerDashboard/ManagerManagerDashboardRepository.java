...
public interface ManagerManagerDashboardRepository extends AbstractRepository {
	...
	@Query("select count(us) from UserStory us where us.manager.id = :id and us.priority = acme.entities.project.Priority.MUST")
	Long totalNumberOfUserStoriesWithMustPriorityByManagerId(int id);

	@Query("select count(us) from UserStory us where us.manager.id = :id and us.priority = acme.entities.project.Priority.SHOULD")
	Long totalNumberOfUserStoriesWithShouldPriorityByManagerId(int id);

	@Query("select count(us) from UserStory us where us.manager.id = :id and us.priority = acme.entities.project.Priority.COULD")
	Long totalNumberOfUserStoriesWithCouldPriorityByManagerId(int id);

	@Query("select count(us) from UserStory us where us.manager.id = :id and us.priority = acme.entities.project.Priority.WONT")
